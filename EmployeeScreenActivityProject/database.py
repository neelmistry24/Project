...
class ActivityDatabase:
    ...
    def cleanup_old_screenshots(self, keep_limit: int = 120, delete_count: int = 60) -> list[str]:
        with self.connect() as conn:
            total = conn.execute(
                "SELECT COUNT(*) as total FROM activity_logs WHERE screenshot_path != ''"
            ).fetchone()["total"]
            ...
            paths = [row["screenshot_path"] for row in old_rows]

            conn.executemany(
                "UPDATE activity_logs SET screenshot_path = '' WHERE id = ?",
                [(id_value,) for id_value in ids],
            )
